...
<div class="container mx-auto py-4">
    ...
    <form action="{{ route('annees.update', $annee) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="date" name="debut" id="debut" class="border rounded w-full p-2" value="{{ old('debut', $annee->debut) }}">
            @error('debut')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label for="fin" class="block">Fin</label>
            <input type="date" name="fin" id="fin" class="border rounded w-full p-2" value="{{ old('fin', $annee->fin) }}">
            @error('fin')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
        </div>
        <div class="flex items-center">
            <input type="checkbox" name="en_cours" id="en_cours" value="1" @if(old('en_cours', $annee->en_cours)) checked @endif>
