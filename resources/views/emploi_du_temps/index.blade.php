            ...
            <div class="container mx-auto p-4">
                ...
                @if (Auth::user()->role->libelle === 'coordinateur')
                    <form method="GET" class="mb-6">
                        <label for="classe_id" class="block text-sm font-medium text-gray-200 mb-1">Filtrer par classe
                            :</label>
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white">
                    <table class="w-full text-sm text-left text-gray-700">
                        ...
                        <tbody>
                            @forelse ($emplois as $emploi)
                                @foreach ($emploi->seances as $seance)
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4">{{ \Carbon\Carbon::parse($seance->date)->format('H:i') }}
                                        </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center text-gray-500">Aucune séance
                                        trouvée.</td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="flex justify-between items-center mt-4">
                    <div class= "flex justify-between items-center ">
                        <a href="{{ route('dashboard') }}" class="bg-blue-600 text-white px-4 py-2 rounded mt-4">Retour
                            au
                            dashboard</a>
                    </div>
                    <div>
                        <a href="{{ route('emploi_du_temps.create') }}"
                            class="bg-blue-600 text-white px-4 py-2 rounded mb-4 inline-block">Ajouter un emploi du
                            temps</a>
                    </div>
                </div>
            </div>
