<x-app-layout>
    <div class="container mx-auto py-8">
        ...
        <a href="{{ route('etudiants.create') }}"
            class="mb-4 inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Ajouter un étudiant</a>
        <table class="min-w-full bg-white border">
            <thead>
                <tr>
                    <th class="py-2 px-4 border">Photo</th>
                    <th class="py-2 px-4 border">Nom</th>
                    <th class="py-2 px-4 border">Prénoms</th>
                ...
                @foreach ($etudiants as $etudiant)
                    <tr>
                        <td class="py-2 px-4 border">
                            @if ($etudiant->user->photo)
                                <img src="{{ asset('storage/' . $etudiant->user->photo) }}"
                                    alt="Photo de {{ $etudiant->user->nom }}" class="w-12 h-12 rounded-full">
                            @else
                        ...
                        <td class="py-2 px-4 border flex gap-2">
                            ...
                            <button data-modal-target="modal-{{ $etudiant->id }}"
                                data-modal-toggle="modal-{{ $etudiant->id }}"
                                class="px-2 py-1 bg-red-600 text-white rounded">Supprimer</button>
                            <div id="modal-{{ $etudiant->id }}" tabindex="-1"
                                ...
                                <div class="bg-white rounded-lg shadow p-6 w-full max-w-md">
                                    ...
                                    <form method="POST" action="{{ route('etudiants.destroy', $etudiant) }}">
                                        ...
                                        <button type="button" data-modal-hide="modal-{{ $etudiant->id }}"
