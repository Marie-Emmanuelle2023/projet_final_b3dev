            ...
            <tbody>
                @forelse ($coordinateurs as $coordinateur)
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            <a href="{{ route('coordinateurs.show', $coordinateur) }}"
                                class="btn btn-info btn-sm">Voir</a>
                            <a href="{{ route('coordinateurs.edit', $coordinateur) }}"
                                class="btn btn-warning btn-sm">Modifier</a>
                            <button data-modal-target="coordinateur-modal-{{ $coordinateur->id }}"
                                data-modal-toggle="coordinateur-modal-{{ $coordinateur->id }}"
                                class="btn btn-danger btn-sm" type="button">Supprimer</button>
                            <!-- Flowbite Modal -->
                            <div id="coordinateur-modal-{{ $coordinateur->id }}" tabindex="-1"
                                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow-sm">
                                        <button type="button"
                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                                            data-modal-hide="coordinateur-modal-{{ $coordinateur->id }}">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500">Voulez-vous vraiment
                                                supprimer le coordinateur <span
                                                    class="font-bold">{{ $coordinateur->user->nom }}
                                                    {{ $coordinateur->user->prenom }}</span> ?</h3>
                                            <form action="{{ route('coordinateurs.destroy', $coordinateur) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button data-modal-hide="coordinateur-modal-{{ $coordinateur->id }}"
                                                    type="submit"
