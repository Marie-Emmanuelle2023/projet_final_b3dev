    ...
    <div class="max-w-md mx-auto mt-10 bg-white p-8 rounded shadow">
        ...
        <form action="{{ route('seances.update', $seance) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" id="date" class="w-full border rounded px-3 py-2 mt-1" required value="{{ old('date', $seance->date) }}">
                @error('date')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="salle" id="salle" class="w-full border rounded px-3 py-2 mt-1" required value="{{ old('salle', $seance->salle) }}">
                @error('salle')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <select name="type_cours_id" id="type_cours_id" class="w-full border rounded px-3 py-2 mt-1" required>
                    ...
                    @foreach($typeCours as $type)
                        <option value="{{ $type->id }}" {{ old('type_cours_id', $seance->type_cours_id) == $type->id ? 'selected' : '' }}>{{ $type->libelle }}</option>
                    @endforeach
                </select>
                @error('type_cours_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div class="mb-4">
                ...
                <select name="classe_id" id="classe_id" class="w-full border rounded px-3 py-2 mt-1" required>
                    ...
                    @foreach($classes as $classe)
                        <option value="{{ $classe->id }}" {{ old('classe_id', $seance->classe_id) == $classe->id ? 'selected' : '' }}>{{ $classe->nom }}</option>
                    @endforeach
                </select>
                @error('classe_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div class="mb-4">
                ...
                <select name="emploi_du_temps_id" id="emploi_du_temps_id" class="w-full border rounded px-3 py-2 mt-1" required>
                    <option value="">Sélectionner</option>
                    @foreach($emplois as $emploi)
                        <option value="{{ $emploi->id }}" {{ old('emploi_du_temps_id', $seance->emploi_du_temps_id) == $emploi->id ? 'selected' : '' }}>{{ $emploi->id }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="module_id" id="module_id" class="w-full border rounded px-3 py-2 mt-1">
                    ...
                    @foreach($modules as $module)
                        <option value="{{ $module->id }}" {{ old('module_id', $seance->module_id) == $module->id ? 'selected' : '' }}>{{ $module->nom }}</option>
                    @endforeach
                </select>
                @error('module_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
