                ...
                <table class="min-w-full bg-white border">
                    ...
                    <tbody>
                        @foreach ($justifications as $justification)
                            <tr>
                                <td class="py-2 px-4 border">{{ $justification->etudiant->user->nom ?? '' }}
                                    {{ $justification->etudiant->user->prenom ?? '' }}</td>
                                <td class="py-2 px-4 border">{{ $justification->motif }}</td>
                                <td class="py-2 px-4 border">
                                    @if ($justification->preuve)
                                        <a href="{{ asset($justification->preuve) }}" class="text-blue-600 underline"
                                ...
                                <td class="py-2 px-4 border flex gap-2">
                                    ...
                                    <button data-modal-target="modal-{{ $justification->id }}"
                                        data-modal-toggle="modal-{{ $justification->id }}"
                                        class="px-2 py-1 bg-red-600 text-white rounded">Supprimer</button>
                                    <div id="modal-{{ $justification->id }}" tabindex="-1"
                                        ...
                                        <div class="bg-white rounded-lg shadow p-6 w-full max-w-md">
                                            ...
                                            <form method="POST"
                                                action="{{ route('justifications.destroy', $justification) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="px-4 py-2 bg-red-600 text-white rounded mr-2">Supprimer</button>
                                                <button type="button" data-modal-hide="modal-{{ $justification->id }}"
