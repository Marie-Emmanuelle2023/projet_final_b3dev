                ...
                <table class="min-w-full bg-white border">
                    ...
                    <tbody>
                        @foreach ($parentEtudiants as $parentEtudiant)
                            <tr>
                                <td class="py-2 px-4 border">{{ $parentEtudiant->parentModel->user->nom ?? '' }}
                                    {{ $parentEtudiant->parentModel->user->prenom ?? '' }}</td>
                                <td class="py-2 px-4 border">{{ $parentEtudiant->etudiant->user->nom ?? '' }}
                                    {{ $parentEtudiant->etudiant->user->prenom ?? '' }}</td>
                                <td class="py-2 px-4 border flex gap-2">
                                    <a href="{{ route('parents.show', $parentEtudiant) }}"
                                        class="px-2 py-1 bg-green-500 text-white rounded">Voir</a>
                                    <a href="{{ route('parents.edit', $parentEtudiant) }}"
                                        class="px-2 py-1 bg-yellow-500 text-white rounded">Modifier</a>
                                    <!-- Modal Flowbite Suppression -->
                                    <button data-modal-target="modal-{{ $parentEtudiant->id }}"
                                        data-modal-toggle="modal-{{ $parentEtudiant->id }}"
                                        class="px-2 py-1 bg-red-600 text-white rounded">Supprimer</button>
                                    <div id="modal-{{ $parentEtudiant->id }}" tabindex="-1"
                                        ...
                                        <div class="bg-white rounded-lg shadow p-6 w-full max-w-md">
                                            ...
                                            <form method="POST"
                                                action="{{ route('parents.destroy', $parentEtudiant) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="px-4 py-2 bg-red-600 text-white rounded mr-2">Supprimer</button>
                                                <button type="button"
                                                    data-modal-hide="modal-{{ $parentEtudiant->id }}"
