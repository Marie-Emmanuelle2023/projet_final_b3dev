<x-app-layout>
    ...
    <div class="max-w-6xl mx-auto mt-10 bg-white p-8 rounded shadow">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('users.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded">+ Ajouter un
                utilisateur</a>
        </div>
            ...
            <tbody>
                @forelse ($users as $user)
                    <tr>
                        ...
                        <td class="border px-4 py-2">{{ $user->identifiant }}</td>
                        <td class="border px-4 py-2">{{ $user->role->libelle ?? '—' }}</td>
                        <td class="border px-4 py-2 flex gap-2">
                            ...
                            <button data-modal-target="user-modal-{{ $user->id }}"
                                data-modal-toggle="user-modal-{{ $user->id }}" class="text-red-600 hover:underline"
                                type="button">
                                Supprimer
                            </button>
                            <!-- Flowbite Modal -->
                            <div id="user-modal-{{ $user->id }}" tabindex="-1"
                                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow-sm">
                                        <button type="button"
                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                                            data-modal-hide="user-modal-{{ $user->id }}">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500">Voulez-vous vraiment
                                                supprimer l'utilisateur <span class="font-bold">{{ $user->nom }}
                                                    {{ $user->prenom }}</span> ?</h3>
                                            <form action="{{ route('users.destroy', $user) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button data-modal-hide="user-modal-{{ $user->id }}" type="submit"
                                                    class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                    Oui, je confirme
                                                </button>
                                            </form>
                                            <button data-modal-hide="user-modal-{{ $user->id }}" type="button"
