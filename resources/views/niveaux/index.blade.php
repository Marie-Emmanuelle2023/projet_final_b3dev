<x-app-layout>
    <div class="max-w-4xl mx-auto mt-10 bg-white p-8 rounded shadow">
        ...
        <a href="{{ route('niveaux.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded mb-4 inline-block">Ajouter
            un niveau</a>
                ...
                @forelse ($niveaux as $niveau)
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            ...
                            <button data-modal-target="niveau-modal-{{ $niveau->id }}"
                                data-modal-toggle="niveau-modal-{{ $niveau->id }}" class="btn btn-danger btn-sm"
                                type="button">Supprimer</button>
                            <!-- Flowbite Modal -->
                            <div id="niveau-modal-{{ $niveau->id }}" tabindex="-1"
                                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow-sm">
                                        <button type="button"
                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                                            data-modal-hide="niveau-modal-{{ $niveau->id }}">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-gray-500">Voulez-vous vraiment
                                                supprimer le niveau <span class="font-bold">{{ $niveau->nom }}</span> ?
                                            </h3>
                                            <form action="{{ route('niveaux.destroy', $niveau) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button data-modal-hide="niveau-modal-{{ $niveau->id }}"
                                                    type="submit"
                                            ...
                                            <button data-modal-hide="niveau-modal-{{ $niveau->id }}" type="button"
