    ...
    <div class="flex min-h-screen bg-gray-100">
        ...
        <main class="flex-1 p-8">
            <div class="container mx-auto py-8">
                ...
                @endif
                <a href="{{ route('professeur_modules.create') }}"
                    class="mb-4 inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Nouvelle
                    affectation</a>
                <div class="overflow-x-auto shadow rounded-lg">
                    <table class="min-w-full bg-white border border-gray-200">
                        ...
                        <tbody>
                            @forelse($professeurModules as $affectation)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="py-2 px-4 border">
                                        ...
                                        @endphp
                                        <span
                                            class="font-semibold text-gray-800">{{ $nomComplet ?: 'Nom inconnu' }}</span>
                                    </td>
                                    <td class="py-2 px-4 border">{{ $affectation->module->nom ?? 'Nom inconnu' }}</td>
                                    <td class="py-2 px-4 border flex gap-2">
                                        <a href="{{ route('professeur_modules.show', $affectation) }}"
                                            class="px-2 py-1 bg-green-500 text-white rounded hover:bg-green-600">Voir</a>
                                        <a href="{{ route('professeur_modules.edit', $affectation) }}"
                                            class="px-2 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Modifier</a>
                                        <form action="{{ route('professeur_modules.destroy', $affectation) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-4 px-4 text-center text-gray-500">Aucune affectation
                                        trouvée.</td>
                                </tr>
                            @endforelse
                        </tbody>
